...
def GLE(iter=len(X), tree_min=0, tree_max=len(X), daymin=105, daymax=263, hspa=10):
			...
			for n in xrange(tree_min, tree_max):
				...
				for i in xrange(iter):
					...
					if (np.abs(X[i]-npos[n])-(HE[i]/np.tan(SolElvNBGW(h_day[h],day))) and np.abs(Y[i]-opos[n])-(HE[i]/np.tan(SolElvNBGW(h_day[h],day)))) < 0:
						...
						KMatrixCrown[i] = (([X[i], Y[i]]+np.dot(RM(-np.radians(SolAziNBGW(float(h_day[h]),day)-210)),[-CW[i]/200,HE[i]])/np.tan(SolElvNBGW(h_day[h],day)))-[npos[n], opos[n]])/(([X[i], Y[i]]+np.dot(RM(-np.radians(SolAziNBGW(float(h_day[h]),day)-210)),[CW[i]/200,HE[i]])/np.tan(SolElvNBGW(h_day[h],day)))-[npos[n], opos[n]])
						...
						KMatrixHeight[i] = (([X[i], Y[i]]+np.dot(RM(-np.radians(SolAziNBGW(float(h_day[h]),day)-210)),[0,HE[i]])/np.tan(SolElvNBGW(h_day[h],day)))-[npos[n], opos[n]])/(([X[i], Y[i]]+np.dot(RM(-np.radians(SolAziNBGW(float(h_day[h]),day)-210)),[0,HE[i]-(CH[i]/100)])/np.tan(SolElvNBGW(h_day[h],day)))-[npos[n], opos[n]])
				...
				if (iter - KMatrixTot.sum()) <= 0: #############This probably should be 'iter - 2 - KMatrixTot.sum()' rather than '-5' from comparison with Wolfram code
					KMatrixTotTot[n] = BeamNBGW(h_day[h],day)
					#print("NotShade")
				else:
					#print("Shade")
					KMatrixTotTot[n] = 0
				...
				GillyBottom[n] += BeamNBGW(h_day[h],day)
	...
	############################################################################
	tfinal = time.time()
	return GillyLight, daymax, hspa, tfinal, tstart
	
############################################################################
############################################################################
#Solve the GLE with a given input parameters

GillyLight, Days, TemporalSpacing, tfinal, tstart = GLE(473, 420, 473, 105, 263, 120)
print("\n", GillyLight)
...
with open("rawdata/GLEOutputTree_8.csv", "wb") as output:
